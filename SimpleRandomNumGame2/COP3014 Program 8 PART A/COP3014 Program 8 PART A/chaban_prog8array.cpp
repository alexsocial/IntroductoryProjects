...
void Test(numberGame*& gameList, int count, int& size);
//Precondition: Takes in a dynamic array gameList that is editable, a changeable size, and a count.
//Postcondition: Successfully will call DoubleSize when count is the same as size.
//Description:  Calls DoubleSize function when count is the same as the size

void DoubleSize(numberGame*& gameList, int count, int& size);
//Precondition: Takes in a dynamic array gameList that is editable, a changeable size, and a count.
//Postcondition: Changes the size of gameList to make it double.
//Description:  doubles the size capacity of gameList
...
//the name, level, and whether or not they won or lost that round
{
	...
	cout << "Enter the filename (make sure it has a .txt to it): ";
	cin >> filename;
...
//Description:  Calls DoubleSize function when count is the same as the size
{
	if (count == size)
	{
		DoubleSize(gameList, count, size);
	}
}
void DoubleSize(numberGame*& gameList, int count, int& size)
//Description:  doubles the size capacity of gameList 
{
	...
	numberGame* temp = new numberGame[size];
		...
		gameList = temp;
	//Use a loop to copy all the numberGames in the gameList over to temp (one by one) 
	//NOTE: there are only “count” number of elements in the gameList  
	//after copying one by one from gameList to temp delete gameList  
	//assign temp to gameList –
	// you will now have a gameList that is double the size
}